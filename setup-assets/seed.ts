...
async function main() {
  ...
  const rfq = await prisma.rfq.create({
    data: {
      ...
      description: 'Looking for reliable matcha supply for our 12 cafe locations. Need ceremonial grade for traditional service and culinary grade for lattes.',
      destinationCountry: 'US',
      destinationCity: 'San Francisco',
      preferredIncoterm: Incoterm.DDP,
      targetDeliveryDate: new Date(Date.now() + 45 * 24 * 60 * 60 * 1000), // 45 days from now
  ...
  await prisma.cart.create({
    data: {
      ...
      items: {
        create: [
          {
            skuId: createdProducts[2].skus[0].id, // Classic Usucha 30g tin
            qty: 10,
            unit: 'unit',
          },
        ],
